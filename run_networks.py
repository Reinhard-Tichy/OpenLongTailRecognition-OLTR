...
class model():
    ...
    def init_models(self, optimizer=True):
        ...
        for key, val in networks_defs.items():
            ...
            self.networks[key] = source_import(
                def_file).create_model(*model_args)
            self.networks[key] = nn.DataParallel(
                self.networks[key]).to(self.device)

            if 'fix' in val and val['fix']:
                print(
                    'Freezing feature weights except for modulated attention weights (if exist).')
    ...
    def init_criterions(self):
        ...
        for key, val in criterion_defs.items():
            ...
            self.criterions[key] = source_import(
                def_file).create_loss(*loss_args).to(self.device)
    ...
    def train(self):
        ...
        self.save_model(epoch, best_epoch, best_model_weights,
                        best_acc, centroids=best_centroids)
    ...
    def eval(self, phase='val', openset=False):
        ...
        self.total_logits = torch.empty(
            (0, self.training_opt['num_classes'])).to(self.device)
    ...
    def centroids_cal(self, data):
        ...
        centroids /= torch.tensor(class_count(data)
                                  ).float().unsqueeze(1).cuda()
