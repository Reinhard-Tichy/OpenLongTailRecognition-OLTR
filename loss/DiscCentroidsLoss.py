...
class DiscCentroidsLoss(nn.Module):
    ...
    def forward(self, feat, label):
        ...
        loss_attract = self.disccentroidslossfunc(
            feat.clone(), label, self.centroids.clone(), batch_size_tensor).squeeze()
        ...
        distmat = torch.pow(feat.clone(), 2).sum(dim=1, keepdim=True).expand(batch_size, self.num_classes) + \
            torch.pow(self.centroids.clone(), 2).sum(
        ...
        # margin = 50.0
        margin = 10.0
        loss_repel = torch.clamp(
            margin - distmat_neg.sum() / (batch_size * self.num_classes), 0.0, 1e6)
...
class DiscCentroidsLossFunc(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_centroids.scatter_add_(0, label.unsqueeze(
